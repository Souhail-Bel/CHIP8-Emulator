...
void OP_UNDF(void){
	writeMessage(0, "Undefined opcode, got %04X", currOPCODE);
...
// CLS: clear display
void OP_00E0(void){
	memset(g_chip8.gfx, 0, DISP_S * sizeof(uint8_t));
	g_chip8.flag_draw = 1;
	g_chip8.PC += 2;
}
...
void OP_8XY4(void){
	...
	g_chip8.V[0xF] = (sum > 0xFF);
	g_chip8.V[X] = sum & 0xFF;
	g_chip8.PC += 2;
	...
	for(int line_y = 0; line_y < height; line_y++)
		for(int line_x = 0; line_x < 8; line_x++)
			if(g_chip8.memory[g_chip8.I + line_y] & (0x80 >> line_x)){
				idx = (((cy+line_y) * DISP_W) + (cx+line_x)) % DISP_S;
